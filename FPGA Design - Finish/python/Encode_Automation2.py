            ...
            if(Encode == 1):    # Instruction Encode
                ...
                if(not(instruction_list == [])):
                    ...
                    # -- BUS - Type Instructions -- #
                        ...
                        Instruction_Encode = BUS_Type_Encode(instruction_list,opcode = 57)
                    # -- BUS - Type Instructions -- #
...
def BUS_Type_Encode(instruction_list,opcode):
    ...
    if(instruction_list[0] == 'PERBUSW'):
        rd = 0
        rs = int(instruction_list[1].replace('R',''))
        address = int(instruction_list[2])
        print(f'opcode = {opcode} | rd = {rd} | rs = {rs} | address = {address}')
    else: # (instruction_list[0] == 'PERBUSR')
        rd = int(instruction_list[1].replace('R',''))
        rs = 0
        address = int(instruction_list[2])
        print(f'opcode = {opcode} | rd = {rd} | rs = {rs} | address = {address}')
    Instruction_Encode = (opcode * (2**26)) + (rd * (2**21)) + (rs * (2**16)) + (address)
    return (Instruction_Encode)
